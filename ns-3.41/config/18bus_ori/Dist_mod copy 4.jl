...
function handle_clients(udpsock)
        ...
        try
                ...
                while iter <= MAX_ITER
                        ...
                        # Collect results from each RPi/Area
                        # println("tx_time +Thr1:", (window_time +Thr))
                        # println("time1:", time())
                        before_time1= time()
                        ...
                        while time()<= (window_time +Thr) && length(received_ids) < num_RPi
                                ...
                                current_time = time()
                                println("after loop:", current_time)
                                println("after loop time ", round((current_time-send_time), digits = 4))
