...
# Ensure PyError is accessible
PyError = PyCall.PyError
# Define the Python socket timeout exception globally
# const timeout_exception = socket.timeout
        ...
        try
                ...
                while count(!isnothing, address_list) < num_RPi
                        ...
                        if isnothing(address_list[rpi_id])
                                ...
                                if rpi_id in 1:num_RPi
                                        julia_addr = Sockets.InetAddr{IPv4}(Sockets.IPv4(addr[1]), addr[2])

                                        address_list[rpi_id] = julia_addr  # Store the address based on RPI ID
                ...
                while iter <= MAX_ITER
                        ...
                        for i in 1:num_RPi
                                ...
                                #conver to python handable address
                                python_addr = (string(ip), port)

                                sent = udpsock.sendto(final_message_bytes, python_addr)
                        ...
                        # global window_time = time() + Thr

                        #implement the timeout in second
                        # println("time_out")
                        #println("received_ids", received_ids)
                        #println("num_RPi", num_RPi)
                        before_time1 = time()
                        println("before loop time1: ", round((before_time1-send_time),digits = 4))
                        
                        # Set the socket to non-blocking mode
                        udpsock.setblocking(0)
                        while ((time()-send_time)< Thr) && (length(received_ids) < num_RPi)
                                # println("Checking condition: Time left = $(time()-send_time), Received IDs = $(length(received_ids))")
                                # udpsock.settimeout(7)
                                # println("check point")
                                try                        
                                        # print("pass3")
                                        # println("data")
                                        before_time2= time()
                                        # println("before loop time2 ", round((before_time2-send_time),digits = 4))

                                        data, sender = udpsock.recvfrom(1048576)
                                        after_time1 = time()
                                        # println("after loop time ", round((after_time1-send_time), digits = 4))
                                        ...
                                        # Check if the received packet number is the one we expect
                                        if received_pak_num != packet_number
                                                println("Received incorrect packet number: ", received_pak_num, ". Ignoring this packet.")
                                        ...
                                        # println("pass 5")
                                        # println("Error caught, checking type...")
                                        # println("Error details: ", e)
                                        after_time2 = time()
                                        # println("after loop time ", round((after_time2-send_time), digits = 4))
                                        continue
                                        # if isa(e, IOError) && occursin("Resource temporarily unavailable", string(e))
                                        #         # Non-blocking call returned no data
                                        #         continue  # Skip to the next loop iteration
                                        # else
                                        #         rethrow(e)  # If it's not the non-blocking error, rethrow the exception
                                        # end
                                end
                        end
                        conti_time= time()
                        println("continue time: ", round((conti_time-send_time),digits = 4))
                        println("continue")
                        ...
                        if length(received_ids) == num_RPi
                                ...
                                for id in 1:num_RPi
                                        if id ∉ received_ids
                                                ...
                                                println("Data not received from RPI ID $id. Using previous data: ", old_data)
                                                rx_time = time()
                                                # println("rx_time:", rx_time)
                        ...
                        # Reset timeout to a more forgiving or different setting if necessary
                        # udpsock.settimeout(nothing)
                        # udpsock.setblocking(1)
                        println("resetting socket timeout")
                        ...
                        # println("residual_12", residual_12)
               ...
               # Open the log file in read mode
                ...
                open("Data/RPilogfile.txt", "a") do file
                        ...
                        write(file, "ExpID = $(new_experiment_id), Date = $(sys_timestamp), NumRPI = $(num_RPi)\n")
                end
