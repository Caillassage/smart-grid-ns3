...
linedata=xlsread('123bus_W_LTC_singlephs.xlsx','Branch_all','A2:D130');
fromnode=linedata(:,1);
tonode=linedata(:,2);
llen=linedata(:,3)*0.000189394; % in miles
lconf=linedata(:,4); % configuration no.
config=xlsread('123bus_W_LTC_singlephs.xlsx','Config','A2:S17');
...
trdata=xlsread('123bus_W_LTC_singlephs.xlsx','Transformers','A2:P2');
l1=size(trdata);
...
for loop1=1:length(fromnode) % loops over all transformer
    ...
    %% Del-Grounded Wye connection
      if trdata(loop1,9)==2 & trdata(loop1,10)==1
         ntr=trdata(loop1,7)/(trdata(loop1,8)/sqrt(3));
         
         Ztr=(trdata(loop1,11)/100)*(trdata(loop1,8)*trdata(loop1,8)/(trdata(loop1,3)/(1000)));
         Zti=(trdata(loop1,12)/100)*(trdata(loop1,8)*trdata(loop1,8)/(trdata(loop1,3)/(1000)));
         zlineinfo((feederno+loop1)*3-2,12)=num2cell(3);% transformer
         zlineinfo((feederno+loop1)*3-2,13)=num2cell(ntr(1));% del, yg
    ...
    %% Del-Del connection
     ...
     %%  Open Grounded Wye-Delta connection
    
    
    %% 1-phase connection
        
    
    %%% Assinging to zlineifo
...
xlswrite('zinfo.xls',zlineinfo);
 
    
 phsA=zlineinfo(1:3:end,:); 
 phsB=zlineinfo(2:3:end,:);
 phsC=zlineinfo(3:3:end,:);
