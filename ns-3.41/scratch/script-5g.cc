...
NS_LOG_COMPONENT_DEFINE("5G-Script");
...
main(int argc, char* argv[])
{
    ...
    */
    ...
    for (uint32_t i = 0; i < gNbNodes.GetN(); ++i) { 
        const auto gnbAddressAuto = gnbIpIface.GetAddress(i);
        InetSocketAddress sinkSocket (gnbAddressAuto, dlPort);
        PacketSinkHelper dlPacketSinkHelper("ns3::UdpSocketFactory", sinkSocket);
        serverApps.Add(dlPacketSinkHelper.Install(gNbNodes.Get(i)));
    }
    ...
    */
    ...
    for (uint32_t i = 0; i < ueNodes.GetN(); ++i) {
        ...
        Ptr<const NrGnbNetDevice> connectedEnb = nrUeDevice->GetTargetEnb();
        uint32_t gnbId = connectedEnb->GetNode()->GetId();
        std::cout << "UE " << i << " is attached to eNB with ID: " << connectedEnb->GetNode()->GetId() << std::endl;
        
        std::cout << gnbId-numGnbs-2 << std::endl;
        Address gnbAddress = gnbIpIface.GetAddress(gnbId-numGnbs-2);
        UdpClientHelper dlClient (gnbAddress, dlPort);
        dlClient.SetAttribute("MaxPackets", UintegerValue(0xFFFFFFFF));
        dlClient.SetAttribute("Interval", TimeValue(Seconds(periodSeconds)));
        ...
        ApplicationContainer clientApps = dlClient.Install(ue);
  ...
  // *** Création du flowMonitor et lancement de la simulation ***
    ...
    if (trafficDirection == "upstream") {
        for (uint32_t index = 0; index < serverApps.GetN (); ++index) {
            totalPacketsThrough = DynamicCast<PacketSink> (serverApps.Get (index))
                                                            ->GetTotalRx ();
    ...
    double totalSentPackets = (( 1 / periodSeconds * simulationTime ) * numUes); // Estimation of number of generated packets in the network
